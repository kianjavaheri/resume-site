...
- **`.project-card-meta` takes a 2px `padding-top`, and that is not a fudge.** Top-aligning the two *boxes* isn't the same as aligning the two *texts*: the label's line box is 13px against the title's 19.76px, so at a shared top edge the label's caps sat 1.8px high. 2px levels the cap heights, which is what "flush with the title" means to the eye. **Cap tops, not baselines** — those are 5.4px apart, and matching them would drop the small label visibly below the title. Measured after: cap tops within 0.2px on all five cards.
- **Both edges line up: the type label with the title, the tiles with the tags.** The top pair comes from the grid's first row starting at the content box's top; the bottom pair from its last row ending at the content box's bottom, with `.project-body .tag-list` taking `margin-top: auto` so the tags end there too. Measured on all five cards at 1280 and 1000px: bottoms flush to 0.0px, cap tops within 0.2px.
