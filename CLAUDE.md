...
- **The tiles' bottom edge is flush with the tags'.** Both are anchored to the same line: the grid's last row ends at the content box's bottom, and `.project-body .tag-list` takes `margin-top: auto` so the tags end there too. Measured on all six cards at 1280 and 1000px: flush to 0.0px, and the label's centre on the title/description gap's centre to 0.0px.
...
- **Horizontally the pair is centred too**, on desktop: the type and the tiles are each centred in the 200px column, so the label sits over the tiles it belongs to instead of hanging off their left edge, and a one-tile project (Estimating the Wage Effects of a UBI) stays aligned with the two-tile ones. Measured: every card's label and tile row share a centre at x=169. **At ≤768px both revert to `flex-start`** — the areas restack full-width there, and a centred label would float away from the left-aligned title and description below it.
...
**Every row is the same height.** `.sub-cards-stack.work-stack` becomes `display: grid; grid-auto-rows: 1fr` above 769px, so every row resolves to the tallest card's content. Measured before: 176 / 224 / 200 / 200 / 200 — a 48px spread that read as almost-but-not-quite aligned. All six are 222px now. **The number moves whenever a card is added or its copy is edited** — it is the tallest card's height, not a constant, so don't hard-code it anywhere.
...
- Each entry needs an **`icon`**: `paper` (a reading page here), `pdf`, `library`, `site` (a deployed web app — a stroked globe), `github`, `curseforge`. The `works` array is annotated `Array<Omit<WorkProps, 'onOpenPdf'>>` so those names are checked rather than widening to `string`.
...
- Each entry carries both a **`label`** (accessible name + tooltip: "View in ASU Library") and a **`short`** (visible text: "ASU Library"). Keep `short` a subset of `label` — a visible label that isn't part of the accessible name breaks *Label in Name* for anyone using voice control. Current set: `Read`, `PDF`, `Poster`, `ASU Library`, `Live Site`, `GitHub`, `CurseForge`. Where a project has two links, the one a visitor most wants goes first: Material Boxes puts **View CurseForge above View GitHub**, since CurseForge is where the mod is actually downloaded, and Santa Cruz Rental Price Model puts **View Live Site above View GitHub** for the same reason. Two badge types sit **inline at the end of the project title**, not on the type label, so on a wrapping title they follow the last word: a green `.project-release` (the shipped version, e.g. "Version 1.1.0") and then a red `.project-wip`. **Material Boxes is the only card carrying either** — it has both. They **share one rule** and differ only in fill — `#2e8b57` and `#e03e3e`, fixed colours rather than theme tokens, both at the same ~4.3:1 against white, so neither shouts over the other.
...
### The abstract is the landing view, and the paper is behind a button

`thesis` and `basic-income` open on their abstract alone — title block, an `Abstract` section, and a **Read the full paper** button. Pressing it renders every section at once and the button unmounts. `cs-capstone` has no abstract and renders in full on arrival, exactly as all three did before: **the gate is opt-in per paper, not a default**, keyed on whether `paperAbstracts` has an entry for the slug.

- **The abstracts live in `src/content/abstracts.ts`, not in each paper's own module** — same reason as `tables.ts`, `charts.ts` and `survey.ts`. `thesis.ts` and `basic-income.ts` are generated from the PDFs, so an abstract written into them is dropped by the next regeneration.
- **Only the thesis's abstract is the document's own.** The Economics capstone was submitted without one, so **its abstract was written for the reading page** from the paper's own argument and findings. It is hand-maintained and will not follow a revision of the paper — this is flagged in `abstracts.ts` itself, and it is the second place on the site whose text isn't a reproduction of its source (the first being Table 1's corrected trade balance).
...
- **The contents list is gated with it**, since its links would otherwise point at sections that aren't in the DOM. While collapsed it shows the single `Abstract` entry and drops its "Contents" label. **The `<nav>` element itself always renders**, even empty: `.paper-layout` is a `210px minmax(0, 1fr)` grid, and removing the nav would promote the article into the 210px track and jump it sideways on expand.
- **`showFull` is a dependency of the scroll-probe effect.** Expanding puts every heading on the page at once, and without the re-run the probe keeps measuring the collapsed id list until the next scroll event — which on a click that doesn't scroll never comes. `abstract` is in the same list because the id array leads with `abstract` when there is one.
...
- Measured: expanding the thesis yields 9 sections, 6 tables, 4 charts, the survey explorer and **still zero `<img>` elements**; the ToC highlight tracks from Abstract through References at max scroll on both papers. At 375px the button is 192x42 inside the 335px column and neither state opens a sideways scroll.
